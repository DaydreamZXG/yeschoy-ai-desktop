...
新增 `yeschoy-vendor-publish.timer`，每六小时在 UTC 00:30、06:30、
12:30、18:30 调用 `publish_pending.py`，与原下载账号隔离。
下载器仍不能写公开目录。发布器复用原包结构、摘要、来源及版本防倒退检查，
保留主版/朋友版更新健康字段，不改客户端更新清单。
同步锁避免边下载边发布；单来源失败保留原包并继续其他来源。
状态写入 `/var/lib/yeschoy-vendor-publish/last-run.json`；
有未发布来源时服务返回失败，下一次定时仍会重试，没有外部通知接收人。

Codex Mac 新 DMG 必须先在 macOS 完成原生验证，再将匹配摘要的收据放到
`/opt/yeschoy-vendor-sync/receipts/<sha256>.json`。无匹配收据时不会发布新包。
其他五个来源可标记 `client_native_required` 发布，安装前仍由客户端执行原生验签。
这不是全平台无人工参与的自动验签流程。

这套服务每 6 小时检查 Codex 和 Claude Desktop 的官方完整安装包，只下载变化的文件；不依赖 NewAPI、不使用用户账户、不运行安装包。同步服务没有公开发布权限；独立发布任务调用 `publish.py`，也支持操作员单独调用，详见上方。
...
8. 完整候选按来源只保留当前版本和最近一个历史版本；断点文件及收据继续计入预算。部署预算为 6 GiB，达到上限就暂停新增下载，不删除服务器其他数据，也不对公网带宽费用作保证。

## 服务器位置与运行

- 代码及配置：`/opt/yeschoy-vendor-sync`，由 root 管理、服务只读。
- 私有状态：systemd `StateDirectory=yeschoy-vendor-sync`，主机实际位置 `/var/lib/private/yeschoy-vendor-sync`；`/var/lib/yeschoy-vendor-sync` 为 systemd 管理入口。每个来源只保留当前候选和最近一个历史候选。
...
中断留下的本服务断点文件可在下一次检查续传。完整候选自动轮换为两代，断点文件不参与完整包轮换。后续升级脚本前先等本轮结束或停用本服务，核对部署前后文件哈希。
...
4. 再检查私有候选未变化，原子替换 `/apps/catalog.json`。严格遵守 `vendor-download-catalog@v2`：最多六个唯一槽，公开 URL 固定为 `https://ergou.qzz.io`，其他已发布槽保留，自动版本倒退被拒绝。发布定时器随后按来源保留当前对象和最近一个历史对象，删除更老且已不被目录引用的对象。公开目录不包含批准依据、操作员身份、私有路径、账户或密钥。
5. 根据成功提交的非空清单设置标准 health 的 `thirdPartyInstallersPublished`，`updatesPublished` 继续 false，不改 updater 路由。
6. `disable` 先原子发布一个合法的空目录，再把 health 设为 false；最近两代对象保留但不再具有目录权威。客户端无需升级即可看到空目录并自动回退厂商官网。重新执行逐来源 `publish` 即可恢复。

退出 0 输出来源、摘要、校验状态及是否复用；退出 2 输出脱敏错误及 `publication: incomplete, replayRequired: true`，不笼统声称没有发生过提交。复制/清单替换失败保留旧清单；已完成的新不可变对象可能保留供重放。清单是客户端权威，health 若在进程中断后短暂低报，可重放发布或停用命令修复。没有自动降级；`sync.py prune` 与 `publish.py prune` 可在持锁状态下显式执行同一套两代保留规则，正常定时流程也会自动执行。
