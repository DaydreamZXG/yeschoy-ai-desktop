...
RU-008 补充：已识别本机 Developer ID Application 身份；此前 RU-007 的 npm-only 验收器布局问题，通过独立冻结的只读 pytest/Node/React 验收修正，保留 pnpm 与原有测试。证书存在不等于候选包已签名或公证；构建产物应另附源码快照、签名检查和公证状态记录。旧失败报告不回写。

RU-009 / RU-010 补充：2026-08-31 已从 `cfaf7090` 构建 Intel 本机候选 app 与 DMG，实际 Developer ID 签名、hardened runtime、安全时间戳、映像完整性和内容一致性通过，继承的 URL 协议已移除。公证已上传，状态仍为处理中，尚不能对外分发。具体路径、哈希、提交 ID 与最终状态见 `outputs/野菜API-macOS-候选包构建记录.md`。后续验收重跑原有运行时与打包场景，不修改历史报告。
...
- 本次 Intel macOS 候选已取得 Developer ID 签名证据；Apple 公证和票据尚未完成，其他架构及原生安装/启动验证仍缺失。
