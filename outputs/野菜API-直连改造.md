...
1. “最近野菜中转记录”的本地数据源随网关删除而失效，需改为查询服务端 `/api/log/self` 并按 token 归因到工具；前端文案中“进入野菜本地桥”的表述需同步修正。
2. 接入页 UI 重做（状态机收敛、主按钮不再静默禁用）。
3. 七个工具在真机各连一次；Windows 真机验证升级迁移路径与长会话。
4. Claude Desktop 桥的别名映射仍随客户端版本（`claude-desktop-route:v1`）变化；若中转站将来支持模型别名，可再次评估彻底去桥。
