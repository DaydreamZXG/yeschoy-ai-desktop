...
RECIPE = ROOT / "tests" / "work_packages" / "ru062" / "installer.nsi"
...
def receipt() -> dict:
    ...
    assert payload["source"]["verificationReport"] == str(RU061_REPORT.relative_to(ROOT))
    assert payload["source"]["verificationReportSha256"] == sha256(RU061_REPORT)
    verified = json.loads(RU061_REPORT.read_text(encoding="utf-8"))
    assert verified["passed"] is True
    assert verified["sourceDiff"] == {}
    assert payload["source"]["verificationRunId"] == verified["runId"]
...
def test_macos_artifact(tmp_path: Path, monkeypatch) -> None:
    # Apple tooling creates transient validation files. Keep them inside the
    # governed scenario's writable temp root instead of the host TMPDIR.
...
def test_windows_cross_artifact_and_release_boundary() -> None:
    ...
    assert payload["tests"] == {"renderer": 454, "native": 479}
    ...
    assert 'VIProductVersion "0.4.11.0"' in recipe_text
    assert 'WriteRegStr HKCU "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\com.yeschoy.desktop" "DisplayVersion" "0.4.11"' in recipe_text
    ...
    assert re.search(r'^version = "0\.4\.11"$', cargo, re.MULTILINE)
    assert re.search(
