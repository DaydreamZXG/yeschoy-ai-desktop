...
function Preview() {
  const [phase, setPhase] = useState<InstallationProgress["phase"]>(
    ...
    <main data-theme={dark ? "dark" : "light"} className="installer-preview">
      ...
      <div className="installer-preview-grid">
        ...
        <aside className="installer-preview-choice">
          <span>安装后继续</span>
          <h2>{canConnect ? "你的选择" : "可以先安装，再登录"}</h2>
          <dl>
            <dt>应用</dt>
            <dd>Codex Desktop</dd>
            <dt>模型</dt>
      ...
      <nav aria-label="预览状态" className="installer-preview-states">
        <button
          className="subtle-button"
          aria-pressed={canConnect}
          onClick={() => setCanConnect(!canConnect)}
        >
          {canConnect ? "切换为未登录" : "切换为已登录"}
        </button>
        {(
          [
            ["idle", "准备安装"],
