...
connections.schemaVersion = 2;
for (const c of connections.connections) c.models = [];
Object.assign(
  connections.connections.find((c) => c.toolId === "codex_desktop")!,
  {
    ...
    models: [
      { modelId: "gpt-5.6-sol", billingGroup: "国模特价" },
      { modelId: "glm-5.3", billingGroup: "default" },
    ],
    lastRequest: {
      modelId: "gpt-5.6-sol",
      billingGroup: "国模特价",
...
(window as any).__TAURI_INTERNALS__ = {
  ...
  invoke: async (command: string, args: any) => {
    ...
    if (
      command === "dismiss_desktop_exit_prompt" ||
      command === "background_desktop_assistant"
    )
      return null;
...
// Local visual fixture navigation only, never part of the product entry.
if (new URLSearchParams(location.search).get("view") === "setup") {
  const observer = new MutationObserver(() => {
    const card = Array.from(document.querySelectorAll("article")).find(
      (node) => node.querySelector("h2")?.textContent === "Codex Desktop",
    );
    const button = Array.from(card?.querySelectorAll("button") ?? []).find(
      (node) => node.textContent === "调整接入",
    );
    if (button) {
      observer.disconnect();
      button.click();
      if (location.hash === "#models") {
        const modelObserver = new MutationObserver(() => {
          const list = document.querySelector(".model-set-editor");
          if (list?.querySelector("code")) {
            modelObserver.disconnect();
            list.scrollIntoView({ block: "center" });
          }
        });
        modelObserver.observe(document.getElementById("root")!, {
          subtree: true,
          childList: true,
        });
      }
