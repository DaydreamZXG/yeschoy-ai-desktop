...
RECEIPT = ROOT / "outputs/yeschoy-ru022-artifacts.json"
...
def test_windows_tauri_command_follows_locked_gates():
    text = WORKFLOW.read_text(encoding="utf-8")
    install = text.index("pnpm install --frozen-lockfile")
    native = text.index("cargo test --locked --manifest-path src-tauri/Cargo.toml")
    build = text.index(
        "pnpm exec tauri build --config src-tauri/tauri.candidate.conf.json --bundles nsis"
    )
    assert install < native < build
    build_line = text[build : text.index("\n", build)]
    assert "-- --locked" not in build_line
...
def test_final_cross_platform_receipt():
    ...
    assert receipt["releaseUnit"] == "RU-022"
    ...
    assert receipt["boundaries"]["productionRuntimeChangedByRu022"] is False
