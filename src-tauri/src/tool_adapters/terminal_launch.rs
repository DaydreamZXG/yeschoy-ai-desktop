...
use super::{AdapterFailure, ResolvedInstallation};
...
fn mac_shell_command(executable: &str, home: &str) -> String {
    format!(
        "cd -- {} && exec {}",
        quoted_shell(home),
    ...
    match platform {
        Platform::MacOs => {
            Ok(LaunchPlan {
                ...
                new_console: false,
                mac_command: Some(mac_shell_command(&executable, &home)),
...
mod tests {
    ...
    fn terminal_plans_are_closed_and_propagate_injected_launcher_failure() {
        ...
        assert_eq!(
            ...
            Some(
                mac_shell_command(installation.path.to_str().unwrap(), "/native/我的项目").as_str()
            )
        );
