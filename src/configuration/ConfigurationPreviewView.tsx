  ...
  return (
    ...
    >
      ...
      <ConfirmDialog
        ...
        title={`重新打开 ${resultContext?.app ?? application.displayName}`}
        message={`这个应用正在运行。请先保存正在编辑的内容。\n\n继续后，野菜助手只会请求它正常退出，设置完成后再重新打开；不会强制结束进程。若应用拒绝退出，本次不会修改设置。`}
        confirmText="已保存，退出并继续"
        cancelText="暂不接入"
        variant="info"
        pending={applyPhase === "applying"}
        onConfirm={() => {
          if (restartPromptContext !== currentContext.current) {
            setRestartPromptContext(null);
            return;
          }
