const zh = {
  ...
  // 这条以前几乎不会出现（判断读错了进程的环境），现在会真的弹给用户看，
  // 所以必须写成能照着做的步骤，而不是「请移除相关环境变量」。
  externalOverride:
    "你的终端里设置了环境变量（ANTHROPIC_BASE_URL / ANTHROPIC_AUTH_TOKEN / ANTHROPIC_API_KEY 之一），它的优先级比助手写的设置更高——接了也不会生效，所以本次没有改动任何设置。这通常是以前用别家中转站留下的。处理办法：打开终端运行 open -e ~/.zshrc（Windows 在「系统属性 → 环境变量」里看），把以 export ANTHROPIC_ 开头的那几行删掉或注释掉，关掉终端窗口重开，再回来重试。",
