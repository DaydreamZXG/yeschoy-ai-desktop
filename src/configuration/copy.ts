...
const zh = {
  ...
  estimateExampleNote: "缓存型示例，不是账单预测",
  estimateSaving: "约省 {{percent}}%",
  estimateFormula: "1,000 万新输入 + 8,000 万缓存读取 + 1,000 万输出",
  estimateOfficialLabel: "使用官网预计",
  estimateYeschoyLabel: "使用野菜预计",
  estimateNote:
    "按 {{group}} 当前倍率和网站参考换算值 {{fx}} 估算{{tiered}}。 该换算值不是市场汇率。{{cacheNote}}",
...
const en: Copy = {
  ...
  chooseGroupLegend: "Choose a billing group",
  groupIntro:
    "The same model can be priced differently per group. Only the groups enabled for this model are listed below; the description under a group is that group's own introduction and may not cover everything.",
  ...
  estimateExampleNote: "A cache-based example, not a billing forecast",
  estimateSaving: "Save about {{percent}}%",
  estimateFormula: "10M new input + 80M cache reads + 10M output",
  estimateOfficialLabel: "Official estimate",
  estimateYeschoyLabel: "野菜API estimate",
  estimateNote:
    "Estimated from {{group}}'s current ratio and the site's reference rate {{fx}}{{tiered}}. The reference rate is not a market exchange rate. {{cacheNote}}",
...
const tw: Copy = {
  ...
  chooseGroupLegend: "選擇計費分組",
  groupIntro:
    "同一個模型，不同分組有不同價格。下面只列出這個模型已開放的分組；分組下方的說明是分組自身的介紹，不一定列全。",
  ratioPending: "倍率待查詢",
  groupsMissing: "暫未讀到這個模型的可用分組，請重新整理帳戶資料。",
  groupsEmptyHint: "選擇模型後，可查看對應計費分組。",
  pricesLabel: "所選分組價格",
  estimateSummary: "1 億 Token 費用參考",
  estimateExampleNote: "快取型示例，不是帳單預測",
  estimateSaving: "約省 {{percent}}%",
  estimateFormula: "1,000 萬新輸入 + 8,000 萬快取讀取 + 1,000 萬輸出",
  estimateOfficialLabel: "使用官網預計",
  estimateYeschoyLabel: "使用野菜預計",
  estimateNote:
    "按 {{group}} 目前倍率和網站參考換算值 {{fx}} 估算{{tiered}}。該換算值不是市場匯率。{{cacheNote}}",
...
const ja: Copy = {
  ...
  chooseGroupLegend: "課金グループを選択",
  groupIntro:
    "同じモデルでも、グループによって価格が異なります。下にはこのモデルで開放されているグループのみ表示されます。グループ下の説明はグループ自体の紹介であり、すべてが載っているとは限りません。",
  ratioPending: "倍率は確認中",
  groupsMissing:
  ...
  estimateExampleNote: "キャッシュ利用時の例であり、請求の予測ではありません",
  estimateSaving: "約{{percent}}%お得",
  estimateFormula:
    "1,000万新規入力 + 8,000万キャッシュ読み取り + 1,000万出力",
  estimateOfficialLabel: "公式サイトでの概算",
  estimateYeschoyLabel: "野菜APIでの概算",
  estimateNote:
    "{{group}}の現在の倍率とサイトの参考換算レート {{fx}} から概算しています{{tiered}}。この換算レートは市場の為替レートではありません。{{cacheNote}}",
